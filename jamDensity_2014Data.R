...
wbData=dataByBatch(5)[,c("locationIDX","jamsPerKm","mean_elevation","mean_latRange_10","mean_latRange_25","mean_minLatRange_10","mean_minLatRange_25","mean_slope","mean_SPI","mean_UAA","jamCount","channelLength","standAge")]
...
#land use / mgmt
jamDataCats=wbDataCats[wbDataCats$dataTypeIDX==42,][,c("locationIDX","value")]
names(jamDataCats)[2]="mgmt"
jamData=left_join(wbData,jamDataCats)
jamData$isManaged=jamData$mgmt=="YM"

#confinement
jamDataCats=wbDataCats[wbDataCats$dataTypeIDX==38,][,c("locationIDX","value")]
...
# nmgdData=fitData[!fitData$isManaged,]
# 
# jamGlm=glm.nb(jamCount~(lUAA+I(lUAA^2))+mean_latRange_10+offset(log(channelLength)),data=nmgdData,na.action=na.fail,control = glm.control(maxit=500))
# summary(jamGlm)
# plot(nmgdData$jamsPerKm,exp(predict(jamGlm)))
# abline(a=0,b=1)
# points(mgdData$jamsPerKm,exp(predict(jamGlm,newdata=mgdData)),pch="*",cex=2)
...
#jamGlm=glm.nb(jamCount~(isManaged+isConf+mean_elevation)^2*(lUAA+I(lUAA^2))+offset(log(channelLength)),data=fitData,na.action=na.fail,control = glm.control(maxit=500))


jamGlm=glm.nb(jamCount~isManaged+lUAA+I(lUAA^2)+mean_latRange_10+isManaged:mean_latRange_10+offset(log(channelLength)),data=fitData,na.action=na.fail,control = glm.control(maxit=500))


hist(jamGlm$fitted.values)

summary(jamGlm)
dredge(jamGlm,extra="R^2",m.lim=c(2,8))
